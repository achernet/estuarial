...
def worldscope_align(df):
    for count, row in df.iterrows():
        if row['ddate'] is np.nan and row['date'] is not np.nan:
            assert row['seq'] in range(1, 5)
            sh_months = 4 - row['seq']
            row['ddate'] = row['date'] + relativedelta(months=-1*sh_months)
            last_fisc_yr = row['date']
        elif row['ddate'] is np.nan and row['date'] is np.nan:
