...
def get_key_columns(sp_help_results):
    """
    ...
    foreign_key_defs = sp_help_results['constraints'].constraint_type.map(
        lambda x: x.startswith("FOREIGN KEY")
    )
    ...
    if foreign_key_defs.any():
        foreign_keys = (
            sp_help_results['constraints'][foreign_key_defs]
            .constraint_keys
            .values[0]
            .split(", ")
        )
...
def make_function_documentation(sp_help_results, table_name):
    ...
    doc_string += doc_yaml_prefix 
    doc_string += "The table provides the following indexes:\n\n"

    doc_string += doc_yaml_prefix 
    doc_string += (
        sp_help_results.get("indexes", empty_index_df)
        .set_index("index_name").T
        .to_string().replace("\n", "\n" + doc_yaml_prefix) + "\n\n"
    )

    # Print-out on any keys / foreign keys / constraints on the table.
    doc_string += doc_yaml_prefix 
    doc_string += "The table provides the following constraint/keys:\n\n"

    doc_string += doc_yaml_prefix 
    doc_string += (
        sp_help_results.get("constraints", empty_constraint_df)
        .set_index("constraint_name").T
        .to_string().replace("\n", "\n" + doc_yaml_prefix) + "\n\n"
    )
